...
export default function Home() {
  return (
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-160px)] text-center px-6 relative overflow-hidden">
      <div className="max-w-4xl space-y-12 animate-in fade-in slide-in-from-bottom-12 duration-1000">
        <div className="space-y-6">
          ...
          <h1 className="text-6xl md:text-8xl font-black text-slate-800 leading-[1.05] tracking-tighter">
            Celebre com <br />
        ...
        <div className="flex flex-col sm:flex-row gap-6 justify-center pt-8">
          <Link
            href="/login"
            className="group relative inline-flex items-center justify-center h-16 px-12 rounded-[2rem] bg-slate-900 text-white text-[11px] font-black uppercase tracking-[0.3em] shadow-2xl shadow-slate-200 transition-all hover:-translate-y-1.5 hover:shadow-slate-300 active:translate-y-0"
          >
            Entrar no Painel
        ...
        <div className="pt-20 grid grid-cols-2 md:grid-cols-4 gap-8 opacity-40 grayscale group-hover:grayscale-0 transition-all duration-700">
          {[
            ...
            <div key={i} className="text-center">
              <p className="text-xl font-black text-slate-800 tracking-tighter">{s.v}</p>
      ...
      <div className="fixed -z-10 top-0 left-0 w-full h-full overflow-hidden pointer-events-none">
        <div className="absolute top-[10%] right-[10%] w-[50%] h-[50%] bg-brand/5 rounded-full blur-[150px] animate-pulse" />
        <div className="absolute bottom-[-10%] left-[-10%] w-[40%] h-[40%] bg-slate-100 rounded-full blur-[120px]" />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[80%] h-[60%] border-[1px] border-slate-50 rounded-[5rem] opacity-20 pointer-events-none" />
      </div>
    </div>
  )
}
